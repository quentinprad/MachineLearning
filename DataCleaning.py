...
kf = cross_validation.KFold(X_train.shape[0], n_folds=4)
...
Y_predict_2 = test_users[['id']].copy()

Y_predict_0['country'] = RandomForestWithFeatureSelection(X_train, Y_train, X_test)
Y_predict_1['country'] = MultiLayer_Perceptron(X_train, Y_train, X_test)
Y_predict_2['country'] = LogReg(X_train, Y_train, X_test)


final_result = Y_predict_0.append(Y_predict_1).drop_duplicates().sort_values('id')
final_result = final_result.append(Y_predict_2).drop_duplicates().sort_values('id')
